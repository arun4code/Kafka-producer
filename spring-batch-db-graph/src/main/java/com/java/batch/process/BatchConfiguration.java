package com.java.batch.process;
...
public class BatchConfiguration {
	 ...
	 * dataSource.setUsername("root"); dataSource.setPassword("admin");
	 * 
	 * return dataSource; }
	 * 
	 * @Bean public JdbcCursorItemReader<User> reader() { JdbcCursorItemReader<User>
	 * reader = new JdbcCursorItemReader<User>(); reader.setDataSource(dataSource);
	 * reader.setSql("SELECT id,name FROM user"); reader.setRowMapper(new
	 * UserRowMapper());
	 * 
	 * return reader; }
	 * 
	 ...
	 * }
	 * 
	 * @Bean public FlatFileItemWriter<User> writer() { File f = new
	 * File("C:\\\\csv\\\\users.csv"); if(!f.exists()) { try { f.createNewFile(); }
	 ...
	 * setDelimiter(","); setFieldExtractor(new BeanWrapperFieldExtractor<User>() {
